...
import com.dong.util.WebUtilsPro;
...
@ControllerAdvice(basePackages = {"com.dong.system.controller","com.dong.home.controller"})
@Controller
public class shiroControllerAdvice {
    ...
    * */
    ...
    public ModelAndView AuthenticationInfo(HttpServletRequest request){
        ModelAndView modelAndView = new ModelAndView();
        System.err.println("========shiro认证处理："+WebUtilsPro.isAjaxRequest(request));
        if (WebUtilsPro.isAjaxRequest(request)) {
            // 输出JSON
            Map<String,Object> map = new HashMap<>();
            map.put("code", "990");
